...
class AggrSum(nn.Module):
    ...
    def forward(self, H, X_node):
        ...
        mask = mask.float() - torch.unsqueeze(torch.range(0,self.V-1).float().to(mask.device), 1)
        mask = (mask == 0).float()
        # (V, N) * (N, s) -> (V, s)
...
for epoch in range(200):
    ...
    # Get loss
    loss = F.nll_loss(out[train_mask], label_list[train_mask].long().to(device))
    ...
    correct = float(pred[train_mask].cpu().detach().eq(label_list[train_mask]).sum().item())
    ...
    if (epoch+1) % 10 == 0:
        ...
        correct = float(pred[test_mask].cpu().detach().eq(label_list[test_mask]).sum().item())
...
for epoch in range(200):
    ...
    # Get loss
    loss = F.nll_loss(out[train_mask], label_list[train_mask].long())
    _, pred = out.max(dim=1)
